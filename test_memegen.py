...
class TestQuedaParaCatalogoInteiro(Base):
    """A cota do Gemini e por modelo e por dia. Gastar a do modelo de triagem
    nao pode derrubar a geracao, que usa outro modelo com cota propria."""

    def setUp(self):
        Base.setUp(self)
        self._triar, self._gerar = M.triar, M.gerar_json

    def tearDown(self):
        M.triar, M.gerar_json = self._triar, self._gerar
        Base.tearDown(self)

    def _gerador_ok(self):
        def falso(sistema, texto, schema, papel, imagem=None, modelo=None,
                  max_tokens=4000, cachear_sistema=False):
            return {"sugestoes": [{"template_id": "drake", "porque": "p",
                                   "textos": []}]}, {"input": 1, "output": 1}
        return falso

    def test_cota_na_triagem_cai_para_full(self):
        def triar_estourado(*a, **kw):
            raise RuntimeError("Gemini respondeu 429: Quota exceeded for metric "
                               "generate_content_free_tier_requests, limit: 500")
        M.triar = triar_estourado
        M.gerar_json = self._gerador_ok()

        sug, uso = M.sugerir("x", self.catalogo, self.contratos, modo="shortlist")
        self.assertEqual(uso["modo"], "full")          # trocou sozinho
        self.assertEqual(len(sug), 1)                  # e ainda gerou
        self.assertIn("cota", uso["aviso"])
        self.assertEqual(uso["candidatos"], len(self.contratos))  # catalogo inteiro

    def test_erro_que_nao_e_cota_continua_falhando(self):
        def triar_quebrado(*a, **kw):
            raise RuntimeError("Gemini respondeu 400: schema invalido")
        M.triar = triar_quebrado
        M.gerar_json = self._gerador_ok()
        with self.assertRaises(RuntimeError):
            M.sugerir("x", self.catalogo, self.contratos, modo="shortlist")

    def test_sem_problema_nao_ha_aviso(self):
        M.triar = lambda *a, **kw: (["drake"], {"input": 1, "output": 1})
        M.gerar_json = self._gerador_ok()
        _, uso = M.sugerir("x", self.catalogo, self.contratos, modo="shortlist")
        self.assertEqual(uso["modo"], "shortlist")
        self.assertIsNone(uso["aviso"])
